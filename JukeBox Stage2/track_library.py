...
def list_all(): #to list detailed information
    output = ""
    for key in library:
        item = library[key]
...
def get_name(key): #get a track's name
    try:
        item = library[key]
        return item.name
    except KeyError:
...
def get_artist(key): #get a artist name 
    try:
        item = library[key]
        return item.artist
    except KeyError:
...
def get_rating(key): #get a rating of track
    try:
        item = library[key]
        return item.rating
    except KeyError:
...
def set_rating(key, rating): #modify a rate
    try:
        item = library[key]
        item.rating = rating
    except KeyError:
...
def get_play_count(key): #to enumerate a playing count
    try:
        item = library[key]
